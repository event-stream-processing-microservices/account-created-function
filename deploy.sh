...
function package {
  ...
  aws cloudformation package \
     --template-file account-created.yaml \
     --output-template-file account-created-package.yaml \
...
function deploy {
  ...
  aws cloudformation deploy \
     --template-file account-created-package.yaml \
     --stack-name lambda-stack || error_exit "Deployment failed..."

  # Remove the deployment package
  rm ./account-created-package.yaml
}
